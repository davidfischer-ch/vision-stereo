...
{
public:
	// Handle wich represent the USB device.
	// Worry about renumeration... When the firmware proceed a renumeration,
	// the name of the device change and then the handle too.
	// In this case you need to get another handle with a call to CreateFile(...) function.
	HANDLE handle; //!< handle to Cypress driver

public:
	// It enable the firmware reset, but it is precised that the
	// firmware come by download.
	...
	// This function download the firmware into the RAM Memory of the FX2
	// The firmware format need to be *.bix for WIN32 host application and
	// *.hex for UNIX host application :
	//		- First open the firmware file and put the data into a buffer
	//		- Second make a DeviceIOControl request for firmware download
	//		  and send the the firmware to the device
	// The only argument is a pointer to the file (like the filename).
...
// This is the class constructor. A call to this function,
...
//! Firmware must be a .hex on Linux and a .bix on Windows
//
// On windows use there is a second call to the handle creator (CreateFile(...)) because of the renumeration of the FX2.
// It isn't necessary in every case, it depend of your firmware.
...
// It set the USB interface you want to use. number is the number of the interface setting,
// and alternateSetting is the number of the alternateSetting. This function need to be call directly after
// the firmware download. The default Interface Settings choose by windows is (0, 0). If you use another
// remember of set the Interface......
...
// Make a IN BULK transfer. pipeNum is the number of the pipe wich need to be used.
// Remeber that the pipes not all the time correspond to the endpoints.
// For exemple if your device as four endpoints like Endpoint 2, 4, 6 and 8. The
// configuration of teh pipe is like that :
//				Pipe 0 :		Endpoint 2
//				Pipe 1 :		Endpoint 4
//				Pipe 2 :		Endpoint 6
//				Pipe 3 :		Endpoint 8
// this is the same configuration when you make a OUT BULK transfer
unsigned CypressEzUSBDevice::bulkRead(unsigned pipeNum, char *buffer, size_t size)
{
	...
	if (count < 0)
	{
		qDebug() << "CypressEzUSBDevice::bulkRead(" << pipeNum << "," << (unsigned)buffer << "," << size << ")";
		qDebug() << usb_strerror();
		return 0;
	}
...
// See comments of bulkRead absolutely....!!!
unsigned CypressEzUSBDevice::bulkWrite(unsigned pipeNum, const char *buffer, size_t size)
{
	...
	if (count < 0)
	{
		qDebug() << "CypressEzUSBDevice::bulkWrite(" << pipeNum << "," << (unsigned)buffer << "," << size << ")";
		qDebug() << usb_strerror();
		return 0;
	}
	return (unsigned)count;
...
bool CypressEzUSBDevice::GetHandle(void)
{
	#if defined(Q_OS_WIN32)
	...
	if (p->handle == INVALID_HANDLE_VALUE)
	{
		qDebug() << "CypressEzUSBDevice::open(" << name << ") : no valid device found";
		return false;
	}

	#endif
}
void CypressEzUSBDevice::setOverlapped(bool Value)
{
	isOverlapped = Value;
}
